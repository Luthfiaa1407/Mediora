...
<div class="flex min-h-screen">
  <aside class="w-64 bg-transparent shadow-lg p-4">
    ...
    <nav class="space-y-4 mt-4">
      ...
      <button type="button" onclick="window.location.href='faq.php'" class="block w-full text-left font-semibold text-white bg-[#3C949E] rounded px-4 py-2">FAQ</button>
  ...
  <div class="flex-1 flex flex-col">
    ...
    <header class="w-322 h-15 bg-[#9BD8DB] shadow-lg flex items-center gap-4 px-4 py-2">
      <h1 class="px-12 py-3 text-[21px] font-semibold text-[#284B63]">FAQ</h1>
    ...
    <main class="bg-transparent py-4 px-6 mt-4">
      <h1 class="text-xl font-semibold text-[#284B63]">Mengelola FAQ</h1>
      <p class="text-reguler text-[#979797] mb-6">Kelola semua pertanyaan dan jawaban yang tersedia, jalankan operasi bila diperlukan</p>

      <div class="pt-2 text-right">
        <button type="button" onclick="openFaqModal()" class="bg-[#024F55] text-white text-[12px] px-4 py-2 rounded-lg font-medium hover:bg-[#337c84] transition duration-200">
          Tambah FAQ Baru
        </button>
      </div>
            ...
            <table class="w-full text-sm text-left text-gray-500">
              ...
              <tbody class="text-xs text-[#555555] uppercase bg-[#F1F9FB]">
                <tr>
                  ...
                  <td class="px-6 py-4">Apa saja metode pembayaran yang diterima oleh mediora?</td>
                  <td class="px-6 py-4">Pembayaran</td>
                  <td class="px-6 py-4">Mediora menyediakan pembayaran digital melalui...</td>
                  <td class="px-6 py-4">Aktif</td>
                  <td class="px-6 py-4">12/05/2025</td>
                  <td class="px-6 py-4 flex gap-2">
                    <button class="text-[#024F55] hover:text-[#337c84]">Edit</button>
                    <button class="text-red-500 hover:text-red-700">Hapus</button>
                  </td>
                </tr>
                <tr>
                  <td class="px-8 py-6">2</td>
                  <td class="px-6 py-4">Bagaimana cara memesan obat di Mediora?</td>
                  <td class="px-6 py-4">Pemesanan</td>
                  <td class="px-6 py-4">Perhatikan langkah berikut:...</td>
                  <td class="px-6 py-4">Aktif</td>
                  <td class="px-6 py-4">05/05/2023</td>
                  <td class="px-6 py-4 flex gap-2">
                    <button class="text-[#024F55] hover:text-[#337c84]">Edit</button>
                    <button class="text-red-500 hover:text-red-700">Hapus</button>
                  </td>
                ...
                <tr>
                  ...
                  <td class="px-6 py-4">Berapa lama waktu yang dibutuhkan untuk melakukan 
                    pengiriman pembelanjaan dari Mediora?</td>
                  <td class="px-6 py-4">Pengiriman</td>
                  <td class="px-6 py-4">Produk akan dikirim maksimal 2 jam setelah pembayaran sukses...</td>
                  <td class="px-6 py-4">Aktif</td>
                  <td class="px-6 py-4">20/04/2023</td>
                  <td class="px-6 py-4 flex gap-2">
                    <button class="text-[#024F55] hover:text-[#337c84]">Edit</button>
                    <button class="text-red-500 hover:text-red-700">Hapus</button>
                  </td>
...
<div id="tambahFaqModal" class="faq-modal">
  <div class="faq-modal-content">
    <span class="close" onclick="closeFaqModal()">&times;</span>
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold text-[#284B63]">Menambahkan FAQ Baru</h2>
    </div>

    <form>
      <div class="mb-4">
        <label class="block text-sm font-medium text-[#474747] mb-2">PERTANYAAN</label>
        <input 
          type="text" 
      ...
      <div class="flex flex-wrap gap-4 mb-4">
        <div class="flex-1 min-w-[200px]">
          ...
          >
            ...
            <option value="Profil">Profil Mediora</option>
          </select>
        </div>
        
        <div class="flex-1 min-w-[200px]">
          <label class="block text-sm font-medium text-[#474747] mb-2">STATUS</label>
          <select 
            class="w-full h-10 border border-gray-300 rounded-lg px-4 focus:outline-none focus:ring-2 focus:ring-[#9BD8DB] shadow-lg"
          >
      ...
      <div class="mb-4">
        <label class="block text-sm font-medium text-[#474747] mb-2">JAWABAN</label>
        <textarea
      ...
      <button type="button" onclick="closeFaqModal()" class="bg-[#024F55] text-white text-[12px] px-4 py-2 w-full h-10 rounded-lg font-medium hover:bg-[#337c84] transition duration-200">SIMPAN FAQ</button>
    </form>
  </div>
</div>

<script>
  var faqModal = document.getElementById("tambahFaqModal");
